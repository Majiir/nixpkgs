        ...
        type = types.attrsOf (types.listOf (types.submodule {
          options = {
            ...
            text = mkOption {
              type = types.str;
              description = lib.mdDoc ''
                Text of the rule (without `service`, `type`, `control`, `module-path` or `module-argument` fields).
              '';
            };
          };
        }));
    ...
    config = {
      ...
      text = let
        ...
        formatRules = type: pipe cfg.types.${type} [
          ...
          (map (rule: "${type} ${rule.control} ${rule.modulePath} ${formatModuleArguments rule.args} ${removeSuffix "\n" rule.text}"))
          (map (removeSuffix " "))
          (concatStringsSep "\n")
        ];
      in mkDefault ''
      ...
      types = {
        ...
        auth = [
          ...
          (let p11 = config.security.pam.p11; in { name = "p11"; enable = cfg.p11Auth; control = p11.control; modulePath = "${pkgs.pam_p11}/lib/security/pam_p11.so"; args = {
            "${pkgs.opensc}/lib/opensc-pkcs11.so" = true;
          }; text = ''
          ''; })
          (let u2f = config.security.pam.u2f; in { name = "u2f"; enable = cfg.u2fAuth; control = u2f.control; modulePath = "${pkgs.pam_u2f}/lib/security/pam_u2f.so"; args = {
          ...
          ''; }
          (let ussh = config.security.pam.ussh; in { name = "ussh"; enable = config.security.pam.ussh.enable && cfg.usshAuth; control = ussh.control; modulePath = "${pkgs.pam_ussh}/lib/security/pam_ussh.so"; args = {
            ca_file = ussh.caFile;
          ...
          ''; })
          (let yubi = config.security.pam.yubico; in { name = "yubico"; enable = cfg.yubicoAuth; control = yubi.control; modulePath = "${pkgs.yubico-pam}/lib/security/pam_yubico.so"; args = {
            inherit (yubi) mode debug;
            chalresp_path = yubi.challengeResponsePath;
            id = if yubi.mode == "client" then yubi.id else null;
          }; text = ''
          ''; })
          ...
          (optionals ((cfg.unixAuth || config.services.homed.enable) &&
            ...
            [
              ...
              ''; }
              { name = "gnupg"; enable = cfg.gnupg.enable; control = "optional"; modulePath = "${pkgs.pam_gnupg}/lib/security/pam_gnupg.so"; args = {
        ...
        session = [
          ...
          ''; }
          { name = "tty_audit"; enable = cfg.ttyAudit.enable; control = "required"; modulePath = "${pkgs.pam}/lib/security/pam_tty_audit.so"; args = {

  ...
  mkRulesTypeOption = type: mkOption {
    ...
    type = types.attrsOf (types.submodule ({ name, config, ... }: {
      options = {
        ...
        settings = mkOption {
          type = with types; attrsOf (nullOr (oneOf [ bool str int path package ]));
          default = {};
          description = lib.mdDoc ''
    ...
    config = {
      ...
      text = let
        ensureUniqueOrder = type: rules:
          let
            headRules = if rules == [] then [] else [ (head rules) ];
            tailRules = if rules == [] then [] else tail rules;
            ...
            checked = zipListsWith checkPair rules tailRules;
          in headRules ++ checked;

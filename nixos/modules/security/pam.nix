    ...
    config = {
      ...
      text = let
        formatRules = type: pipe cfg.types.${type} [
          ...
          (map (rule: "${type} ${rule.control} ${removeSuffix "\n" rule.text}"))
          (concatStringsSep "\n")
        ];
      in mkDefault ''
        # Account management.
      ...
      types = {
        ...
        auth = [
          ...
          (let u2f = config.security.pam.u2f; in { name = "u2f"; enable = cfg.u2fAuth; control = u2f.control; text = (''
              ${pkgs.pam_u2f}/lib/security/pam_u2f.so ${optionalString u2f.debug "debug"} ${optionalString (u2f.authFile != null) "authfile=${u2f.authFile}"} ''
        ...
        session = [
          ...
          { name = "loginuid"; enable = cfg.setLoginUid; control = if config.boot.isContainer then "optional" else "required"; text = ''
            pam_loginuid.so
          ''; }
          { name = "tty_audit"; enable = cfg.ttyAudit.enable; control = "required"; text = (concatStringsSep " \\\n  " ([
            "${pkgs.pam}/lib/security/pam_tty_audit.so"
